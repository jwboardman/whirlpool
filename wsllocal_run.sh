...
function zk_kafka() {
    ...
    tmux new -d -n Zookeeper -s Zookeeper -x 80 -y 40 ${ZOOKEEPER_HOME}/bin/zookeeper-server-start.sh ${ZOOKEEPER_HOME}/config/zoo.cfg
    ...
    tmux new -d -n Kafka -s Kafka -x 80 -y 40 ${KAFKA_HOME}/bin/kafka-server-start.sh ${KAFKA_HOME}/config/server.properties
...
function start_services {
    ...
    tmux new -d -n StockService -s StockService -x 80 -y 40 ./stockservice/target/stockservice
    echo "Starting UpDownService"
    tmux new -d -n UpDownService -s UpDownService -x 80 -y 40 ./updownservice/target/updownservice
    echo "Starting WeatherService"
    tmux new -d -n WeatherService -s WeatherService -x 80 -y 40 ./weatherservice/target/weatherservice
    echo "Starting WhirlpoolServer"
    cd whirlpoolserver
    tmux new -d -n WhirlpoolServer -s WhirlpoolServer -x 80 -y 40 "./target/whirlpoolserver"
    cd ..
}
...
if [ $? -eq 0 ]; then
   ...
   echo "tmux not found. Please upgrade your WSL install."
   exit 1
fi
